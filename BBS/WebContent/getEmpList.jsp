      	...
      	function makeData(){
			...
			//체크한 행만 전송할 데이터 만들기. 은행계좌가 입력된 경우만 포함함.
      		$("[name='selId']:checked").each(function(i, checkbox){
				...
				var commission_pct = td.eq(4).text();
				...
				//객체에 담기
				obj["slipAmount"] = parseInt(salary)+parseInt(commission_pct);// 급여 + 상여금
			...
			//객체를 string으로 변환
      		...
      		$.ajax({
      			url :'/spring/slip',
      			data :JSON.stringify(list) ,		//[요청] 파라미터(쿼리문자열)
      			type:'post',	//[요청] method/ default는 GET (GET인경우 생략가능) :
      			contentType :'application/json',	//[요청] 파라미터의 타임
      			dataType :'json' ,		//[응답](응답결과 타입)결과를 넘겨받을 타임
      			success : function(result) {
      				alert("전체 건수 :" +result.tcount); //화면에서 체꾸하고 처리한 숫자
      			},
      			error : function(xhi,status) {
      				alert(status);
      			}
      		});
